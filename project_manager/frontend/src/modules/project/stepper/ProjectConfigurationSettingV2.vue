  ...
  <div class="d-flex flex-column justify-space-between" style="height: 88%; width: 100%">
    <div style="width: 100%">
      ...
      <div class="d-flex align-center" style="gap: 25px">
        ...
        <v-radio-group v-model="learningType" row hide-details="" class="ma-0" :disabled="taskType === TaskType.CHAT">
  ...
  methods: {
    ...
    next() {
      this.$emit("next", {
        ...
        dataset: this.taskType === TaskType.CHAT ? "" : this.project.dataset // CHAT인 경우 dataset이 필요없기 때문
      });
    },
